...
export function ConfirmModal({
  open, onClose, onConfirm, mode,
  payAmount, paySymbol, receiveAmount, receiveSymbol, rate, fee,
}: {
  open: boolean; onClose: () => void; onConfirm: () => void;
  mode: "buy" | "sell";
  payAmount: string; paySymbol: string;
  receiveAmount: string; receiveSymbol: string;
  rate: string; fee: string;
}) {
  useEffect(() => {
    if (!open) return;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md bg-card-background border border-border rounded-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-150">
        ...
        <div className="flex items-center justify-between px-5 pt-5 pb-4 border-b border-border">
          ...
          <button onClick={onClose} className="w-7 h-7 rounded-lg flex items-center justify-center text-text hover:bg-border transition-colors">
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="px-5 py-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-background border border-border rounded-xl p-3">
              <p className="text-xs text-text mb-1">{mode === "buy" ? "You Pay" : "You Sell"}</p>
          ...
          <div className="space-y-2 text-xs">
            <div className="flex justify-between text-text">
              <span>{rate.split("=")[0].trim()} = {rate.split("=")[1]?.trim()}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-text">Transaction Fee</span>
              <span className={fee === "0 Fee" || fee === "O Fee" ? "text-green-500 font-medium" : "text-card-text font-medium"}>
                {fee}
              </span>
            </div>
          </div>
