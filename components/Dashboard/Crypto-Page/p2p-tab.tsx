import { P2P_LISTINGS } from "@/lib/MockData";
import {
  ChevronDown,
  Clock,
  SlidersHorizontal,
...
export function P2PPage({ embedded = false }: { embedded?: boolean }) {
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <div className="relative flex-1 min-w-36">
          <input
            ...
            className="w-full h-9 pl-3 pr-3 text-xs border border-input rounded-lg bg-card-background text-card-text placeholder:text-placeholder outline-none focus:border-border-active transition-colors"
          />
        </div>
      ...
      <div className="bg-card-background border border-border rounded-2xl overflow-hidden">
        ...
        <div className="divide-y divide-border">
          {P2P_LISTINGS.map((listing, i) => (
            <div
              key={i}
              className="grid grid-cols-5 gap-4 px-4 py-3.5 hover:bg-border/20 transition-colors items-center"
            >
              {/* Merchant */}
              <div className="flex items-center gap-2 min-w-0">
                ...
                <div className="min-w-0">
                  <p className="text-xs font-semibold text-card-text truncate">
                    {listing.name}
                  </p>
                  <p className="text-[10px] text-text truncate">
                    {listing.stats}
                  </p>
