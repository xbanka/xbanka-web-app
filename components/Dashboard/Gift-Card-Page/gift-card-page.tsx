        ...
        <div className="flex gap-1 border-b border-border sm:border-none">
          {(["overview", "sell", "history"] as GiftTab[]).map((t) => (
            <button
              key={t}
              onClick={() => setTab(t)}
              className={`pb-2 sm:pb-0 px-4 sm:px-5 sm:py-2 text-sm max-sm:text-[12px] font-medium capitalize border-b-2 sm:border sm:rounded-lg transition-colors -mb-px sm:mb-0
                ${
                  tab === t
