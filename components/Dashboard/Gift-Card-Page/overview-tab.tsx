    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[
          ...
          },
          {
            label: "Total Payout",
            value: "₦22,456,235.00",
            sub: "+3.0% vs last week",
            icon: "💰",
            color: "bg-green-500/10 text-green-500",
          },
          {
            label: "Top Traded Card",
            value: "Amazon",
            sub: "35 Traded",
            icon: "🏆",
        ...
        <div className="bg-card-background border border-border rounded-2xl p-5">
          ...
          <div className="grid grid-cols-2 gap-2">
            {CARD_RATES.map((card) => (
              <div
                key={card.name}
                className="border border-border rounded-xl p-3 hover:border-border-active transition-colors cursor-pointer"
              >
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <div>
                    ...
                    <span
                      className={`text-[10px] flex items-center gap-0.5 ${card.up ? "text-green-500" : "text-red-500"}`}
                    >
                      {card.up ? (
                ...
                <div className="flex items-center justify-between">
                  ...
                  <button className="text-xs text-Green font-medium hover:underline">
                    Trade
                  </button>
                </div>
              </div>
      ...
      <div className="bg-card-background border border-border rounded-2xl p-5 max-sm:p-4">
        <div className="flex flex-col gap-3 mb-4">
          <h3 className="font-semibold text-card-text text-sm max-sm:text-[18px] max-sm:leading-7">
            Transaction History
          </h3>
          <div className="flex items-center gap-2 max-sm:flex-col max-sm:items-stretch">
            <div className="relative max-sm:w-full">
              <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-placeholder max-sm:h-5 max-sm:w-5" />
              <input
                placeholder="Search..."
                className="pl-8 pr-3 h-8 text-xs border border-input rounded-lg bg-transparent text-card-text placeholder:text-placeholder outline-none focus:border-border-active w-36 max-sm:h-12 max-sm:w-full max-sm:rounded-xl max-sm:pl-10 max-sm:text-[14px]"
              />
            </div>
            <button className="flex items-center justify-center gap-1.5 h-8 px-3 text-xs border border-input rounded-lg text-card-text hover:border-border-active transition-colors max-sm:h-12 max-sm:w-full max-sm:rounded-xl max-sm:text-[14px]">
              Transaction Type{" "}
              <SlidersHorizontal className="w-3 h-3 text-placeholder max-sm:h-4 max-sm:w-4" />
            </button>
          </div>
        </div>
        <div className="flex gap-1 mb-4 border-b border-border overflow-x-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
          {["All", "Pending", "Completed", "In Progress", "Failed"].map((t) => (
            <button
              key={t}
              className="pb-2 px-3 text-xs font-medium border-b-2 border-transparent text-text hover:text-card-text whitespace-nowrap transition-colors first:border-Green first:text-Green max-sm:px-6 max-sm:text-[14px]"
            >
              {t}
            </button>
          ))}
            ...
            >
              <div className="mb-3 flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <p className="truncate font-mono text-[12px] font-medium leading-5 text-text">
                    {tx.ref}
                  </p>
                  <p className="text-[14px] font-medium leading-5 text-card-text">
                    {tx.cardType} Gift Card
                  </p>
                </div>
                <StatusBadge status={tx.status} />
              </div>

              <div className="grid grid-cols-2 gap-x-4 gap-y-3 text-[12px] leading-5">
                <div>
                  <p className="text-text">Date & Time</p>
                  <p className="font-medium text-card-text">{tx.date}</p>
                  <p className="text-text">{tx.time}</p>
                </div>
                <div>
                  <p className="text-text">Region</p>
                  <p className="font-medium text-card-text">{tx.region}</p>
                </div>
                <div>
                  <p className="text-text">Amount</p>
                  <p className="font-medium text-card-text">{tx.amount}</p>
                </div>
                <div>
                  <p className="text-text">Payout</p>
                  <p className="font-medium text-Green">{tx.payout}</p>
                </div>
              </div>
            </div>
          ))}
