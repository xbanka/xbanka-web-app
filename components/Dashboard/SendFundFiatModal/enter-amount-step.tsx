      ...
      <div className="px-8 pb-3">
        ...
        <div className="flex items-center gap-3 px-3 py-2.5 rounded-lg border border-border bg-background">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm font-medium text-card-text truncate">{recipient.name}</p>
            <p className="text-xs text-text truncate">{recipient.uid}{recipient.bank ? ` · ${recipient.bank}` : ""}</p>
          </div>
        </div>
      </div>
      ...
      <div className="px-8 pb-8 pt-4 space-y-8">
        ...
        <div className="space-y-2">
          ...
          <div className="relative flex items-center">
            <span className="absolute left-3 text-card-text font-semibold text-xl">₦</span>
            <input
          ...
          <div className="flex justify-between text-xs text-text">
            ...
            <span>Balance: ₦{balance.toLocaleString("en-NG", { minimumFractionDigits: 2 })}</span>
          </div>
        </div>
        ...
        <div className="flex gap-3 pt-2">
          ...
          <Button
            className="flex-1"
            disabled={!isValid}
            onClick={() => onContinue(numericAmount, narration)}
          >
            Continue
          </Button>
        </div>
