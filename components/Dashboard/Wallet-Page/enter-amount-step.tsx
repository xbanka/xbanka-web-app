...
export function EnterAmountStep({
  ...
  const nairaEquiv = (inputAmount * 1600).toLocaleString();
  const isValid = inputAmount > 0 && inputAmount + fee <= numericBalance;

  const sendMax = () => {
    const max = Math.max(0, numericBalance - fee).toFixed(2);
    setAmount(max);
  };
    ...
    <Modal className="p-0" onClose={onClose}>
      ...
      <div className="px-8 pt-4 pb-8 space-y-8">
        ...
        <div className="border border-input bg-border rounded-[20px] p-5">
          ...
          <div className="flex items-center justify-between border-b border-input p-3">
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-[12px] font-normal leading-5.5 text-text">Available balance</p>
        ...
        <div className="flex items-center gap-2 text-xs text-text">
          ...
          <span className="text-[12px] font-normal leading-4.5">Secure end-to-end encryption</span>
        </div>
