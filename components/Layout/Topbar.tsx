...
export function Topbar({
  ...
  const openNotifications = useNotificationModalStore(
    (state) => state.open
  );
  ...
  return (
    <header className="flex items-center justify-between px-4 sm:px-8 py-2 border-b border-border bg-card-background shrink-0">
      ...
      <div className="flex items-center gap-4">
        <ThemeToggle />
        <button onClick={openNotifications} className="relative border border-input rounded-lg p-3 bg-border text-card-text hover:bg-border/40 transition-colors">
