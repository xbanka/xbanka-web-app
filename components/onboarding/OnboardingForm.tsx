...
export default function OnboardingForm() {
  ...
  const stepProps = {
    ...
    errors,
    control,
    ...(isSelfieStep ? { onSkip: handleNext } : {}),
  };
      ...
      <div className="min-h-screen bg-background flex items-center justify-center px-4">
        <div className="w-full max-w-lg bg-card-background border border-border rounded-2xl p-14 flex flex-col items-center gap-5 text-center">
          ...
          <p className="text-sm text-text leading-relaxed">
            Your XBanka account is being verified. We'll notify you once it's
            ready.
          </p>
          <button
            onClick={() => {
    ...
    <div className="">
      ...
      <Card>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            <StepComp TOTAL={TOTAL} step={step} handleNext={handleNext} {...stepProps} />
            {!isSelfieStep && (
              <div className="flex flex-col md:flex-row gap-4 mt-1">
                <Button variant={"outline"} size="lg" className="flex-1" onClick={handleNext}>Cancel</Button>
                <Button
                  size="lg"
                  className="flex-3"
